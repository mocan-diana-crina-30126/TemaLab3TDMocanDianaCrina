    ...
    methods: {
        ...
        send: function () {
            if (this.validate(this.dataBits) === true){
                ...
                this.status = 'Input is not valid. Please use 0 or 1 as data bit values';   //daca se adauga altceva in afara de 0 si 1 primim eroare
            }
        },
        encode: function(bits){
           ...
           console.log(matrix2);       //afisare
          
           for(var i=0; i<matrix2.length;i++) {
               bits[i] = matrix2[i];
           }
           var lung = bits.length;
            return [bits,matrix1,widthMatrix,lengthMatrix]; //cuvantul pe care trebuie il transmite
        },
        ...
        validateBit: function(character){
            if (character === null) return false;      //daca nu am adaugat un bit, vom primi eroare
